...
- 📁 **SFTP 文件浏览器** —— 远程/本地双栏文件管理,支持拖拽传输、实时传输队列与懒加载目录树。支持 **多选**(Ctrl / Shift / Ctrl+A),并对整个选区一次性操作 —— 批量下载、批量删除(`rm -rf`,**非空目录照样删**)、把所有路径一次输入到终端。可从工具栏或**右键空白处**新建**文件 / 文件夹**,双击用 **Monaco 编辑器**打开并直接存回服务器;面板贴着终端时,还能**让那个终端 cd 到你正在浏览的目录** —— 首次打开时它也正是从终端当前目录起步的。
